package com.theta360.v2.network;
...
public class HttpConnector {
   ...
   *
   * @param maxReceiveEntry
   *     Maximum number of files that can be acquired at once
   * @param token
   *     Set the previously acquired token to continue. Set null if acquiring for the first time.
   *
  ...
  public Bitmap getThumb(String fileId) {
    ...
    try {
      ...
      parameters.put("maxSize", 200 * 1024);
      input.put("parameters", parameters);
  ...
  public ShootResult takePicture(HttpEventListener listener) {
    ...
    try {
      ...
      if (status.equals("inProgress")) {
        ...
        mCheckStatusTimer.scheduleAtFixedRate(capturedTimerTask,
   ...
   *
   * @param commandId
   *     Command ID for shooting still images
   *
   ...
   *     File ID
   * @param listener
   *     Listener for receiving received data count
   *
   * @return Image data
   */
   ...
   *     File ID
   * @param listener
   *     Listener for receiving deletion results
   */
  public void deleteFile(String deletedFileId, HttpEventListener listener) {
    ...
    try {
      ...
      if (status.equals("inProgress")) {
        ...
        mCheckStatusTimer.scheduleAtFixedRate(deletedTimerTask,
                                              CHECK_STATUS_PERIOD_MS,
                                              CHECK_STATUS_PERIOD_MS);
      } else if (status.equals("done")) {
