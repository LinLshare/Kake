...
public class MJpegView extends SurfaceView implements SurfaceHolder.Callback {
  ...
  private class MJpegViewThread extends Thread {
    ...
    /**
     * Acquire image size according to display area<p> Calculates the size that fits the display
     * area while maintaining the aspect ratio of the motion JPEG.
     *
     * @param bitmapWidth
     *     Width of motion JPEG
     * @param bitmapHeight
     *     Height of motion JPEG
     *
     * @return Image size
     */
            ...
            synchronized (mSurfaceHolder) {
              try {
                if ((mMJpegInputStream != null) && (bitmapCanvas != null)) {
                  Bitmap bmp = mMJpegInputStream.readMJpegFrame();
                  Matrix matrix = new Matrix();
                  matrix.postRotate(-90);
                  dstbmp =
                      Bitmap.createBitmap(bmp, 0, 0, bmp.getWidth(), bmp.getHeight(), matrix, true);
                  bitmapRect = getImageRect(dstbmp.getWidth(), dstbmp.getHeight());
                  bitmapCanvas.drawColor(Color.BLACK);
                  bitmapCanvas.drawBitmap(dstbmp, null, bitmapRect, new Paint());
                  dstbmp.recycle();
                }
              } catch (IOException e) {
                e.getStackTrace();
