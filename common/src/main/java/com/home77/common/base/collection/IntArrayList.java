...
public final class IntArrayList {
  ...
  public IntArrayList add(int value) {
    ...
    if (s == a.length) {
      int[] newArray =
          new int[s + (s < (MIN_CAPACITY_INCREMENT / 2) ? MIN_CAPACITY_INCREMENT : s >> 1)];
  ...
  public int binaryAdd(int value) {
    int index = Arrays.binarySearch(array, 0, size, value);
    if (index < 0) {
      index = ~index;
    }
    add(index, value);
   ...
   * <p/>
   * NOTE: This method is inlined into {@link #add(int)} for performance.
