...
import com.home77.common.base.util.RuntimeInfoHelper;
 ...
 *
 * @see <p>http://stackoverflow.com/questions/2364910/can-i-use-assert-on-android-devices</p>
 */
public final class Check {
  ...
  /**
   * Assert when current is not run on main process.
   */
  public static void currentlyOnMainProcess() {
    Check.d(RuntimeInfoHelper.currentlyOnMainProcess(),
            "Should run on main process[" + RuntimeInfoHelper.mainProcessName() + "]");
  }

  /**
   * Assert when current is not run on main thread.
   */
  public static void currentlyOnMainThread() {
    d(RuntimeInfoHelper.currentlyOnMainThread(), "Should run on main thread");
  }

  /**
   * Assert when current is run on main thread.
   */
  public static void currentlyNotOnMainThread() {
    d(!RuntimeInfoHelper.currentlyOnMainThread(), "Should not run on main thread");
  }
}
